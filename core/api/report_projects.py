...
def report_projects(criteria):
    ...
    # Day mining
    days_result = list(_find_days_by_projects(projects_input, sanified_criteria))
    days_ids_list = [ ObjectId(r['_id']['user_id']) for r in days_result ]
    
    # Salary mining
    salaries_result = list(_find_salaries_by_date_users(days_ids_list, sanified_criteria['end'], sanified_criteria['start']))

    # Project budget extra mining
    budget_result = list(_find_incomes_by_project_date(projects_input, sanified_criteria['end'], sanified_criteria['start']))

    # Cost mining
    costs_result = list(_find_costs_by_project_date(projects_input, sanified_criteria['end'], sanified_criteria['start']))
